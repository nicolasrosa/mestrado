...
def createDatasetHandler(args):
    ...
    if(args.dataset[0:5] == 'kitti'): # If the first five letters are equal to 'kitti'
        ...
        if args.dataset == 'kitti2012':
            # DATASET_PATH = "/media/nicolas/Documentos/workspace/datasets/kitti/data_stereo_flow"
            DATASET_PATH = "/media/olorin/Documentos/datasets/kitti/data_stereo_flow"
...
def main():
    # Local Variables
    DATASET_PATH = None
    imageOutputSize = None
    depthOutputSize = None
    ...
    imageOutputSize, depthOutputSize, DATASET_PATH = createDatasetHandler(args)
    ...
    if args.dataset == 'nyuDepth':
        ...
        for i in range(0, len(list_test_colors_files_path)):
            test_valid_pair_files.append(
                os.path.split(list_test_colors_files_path[i])[1] in list_test_depth_files_filename)
